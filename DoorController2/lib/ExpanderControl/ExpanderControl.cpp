...
ExpanderControl::ExpanderControl()
{
    _i2cPort = NULL;
    _debugPort = NULL;
...
bool ExpanderControl::begin(TwoWire &wirePort)
{
...
boolean ExpanderControl::isConnected(void)
{
    _i2cPort->beginTransmission((uint8_t)_deviceAddress);
    return (_i2cPort->endTransmission() == 0);
}

void ExpanderControl::setDebugStream(Stream &debugPort)
{
    _debugPort = &debugPort;
}

PCA9536_error_t ExpanderControl::pinMode(uint8_t pin, uint8_t mode)
{
    PCA9536_error_t err;
...
PCA9536_error_t ExpanderControl::write(uint8_t pin, uint8_t value)
{
    PCA9536_error_t err;
...
PCA9536_error_t ExpanderControl::digitalWrite(uint8_t pin, uint8_t value)
{
    return write(pin, value);
}

uint8_t ExpanderControl::readReg()
{
    PCA9536_error_t err;
...
uint8_t ExpanderControl::read(uint8_t pin)
{
    PCA9536_error_t err;
    uint8_t inputRegister = 0;
...
uint8_t ExpanderControl::digitalRead(uint8_t pin)
{
    return read(pin);
}

PCA9536_error_t ExpanderControl::invert(uint8_t pin, PCA9536_invert_t inversion)
{
    PCA9536_error_t err;
...
PCA9536_error_t ExpanderControl::revert(uint8_t pin)
{
    return invert(pin, PCA9536_RETAIN);
}

PCA9536_error_t ExpanderControl::readI2CBuffer(uint8_t *dest, PCA9536_REGISTER_t startRegister, uint16_t len)
{
...
PCA9536_error_t ExpanderControl::writeI2CBuffer(uint8_t *src, PCA9536_REGISTER_t startRegister, uint16_t len)
{
    if (_deviceAddress == PCA9536_ADDRESS_INVALID)
    {
        PCA9536_DEBUGLN(STORAGE("ERR (readI2CBuffer): Invalid address"));
...
PCA9536_error_t ExpanderControl::readI2CRegister(uint8_t *dest, PCA9536_REGISTER_t registerAddress)
{
    return readI2CBuffer(dest, registerAddress, 1);
}

PCA9536_error_t ExpanderControl::writeI2CRegister(uint8_t data, PCA9536_REGISTER_t registerAddress)
{
